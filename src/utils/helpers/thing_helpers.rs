...
use crate::world::environment::thing::{Thing, ThingType};
...
pub fn calculate_satisfaction_gain(
    ...
    let base_satisfaction = match resource_type {
        ThingType::Water => 0.5,   // High satisfaction for survival need
        ThingType::Food => 0.4,    // High satisfaction for survival need
        ThingType::Rest => 0.3,    // Moderate satisfaction for recovery
        ThingType::Safety => 0.35, // Moderate satisfaction for security
        ThingType::Social => 0.2,  // Lower but sustained satisfaction
    };
...
pub fn calculate_consumption_rate(
    ...
    let base_consumption = match resource_type {
        ThingType::Water => 0.1,   // Moderate consumption
        ThingType::Food => 0.08,   // Moderate consumption
        ThingType::Rest => 0.0,    // Rest doesn't deplete (hotels are safe zones)
        ThingType::Safety => 0.0,  // Safety doesn't deplete
        ThingType::Social => 0.05, // Light consumption of social resources
    };
...
pub fn calculate_regeneration_amount(
    ...
    let regeneration_rate = match resource.thing_type {
        ThingType::Water => 0.02,  // Steady regeneration (wells refill)
        ThingType::Food => 0.015,  // Slower regeneration (food preparation)
        ThingType::Rest => 0.0,    // Hotels don't regenerate capacity
        ThingType::Safety => 0.0,  // Safety zones don't regenerate
        ThingType::Social => 0.03, // Social spaces recover quickly
    };
...
/// Note: This function is kept for compatibility but the events system has been moved to domain plugins
pub fn resource_matches_need(resource_type: ThingType, need_type: &str) -> bool {
    match (resource_type, need_type) {
        (ThingType::Water, "Thirst") => true,
        (ThingType::Food, "Hunger") => true,
        (ThingType::Rest, "Rest") => true,
        (ThingType::Safety, "Safety") => true,
        (ThingType::Social, "Social") => true,
        _ => false,
    }
}
...
pub fn get_need_level_for_resource(
    ...
    match resource_type {
        ThingType::Water => needs.thirst,
        ThingType::Food => needs.hunger,
        ThingType::Rest => needs.rest,
        ThingType::Safety => needs.safety,
        ThingType::Social => needs.social,
    }
}
